...
def div_KL(S1,S2,resolution=1.0):
    ...
    P[P< 10**(-32)] = 10**(-32)
    #A = P/Q
    #return np.trapz(P*np.log(A)), dx=resolution, axis=1)

    return np.trapz(P*(np.log(P) - np.log(Q)), dx=resolution, axis=1)
...
def pseudo_div_KL(H1, H2, resolution=1.0):
    ...
    k1 = np.trapz(H1,dx = resolution, axis=1).reshape(nbsamples,1)
    k2 = np.trapz(H2,dx = resolution, axis=1).reshape(nbsamples,1)
...
def pseudo_div_KL2(H1, H2, resolution=1.0):
    ...
    ## Spectral integration using trapezoidale rule
    k1 = np.trapz(H1,dx = resolution, axis=1).reshape(nbsamples,1)
    k2 = np.trapz(H2,dx = resolution, axis=1).reshape(nbsamples,1)
    ...
    dist = np.concatenate ( (dist_shape, dist_energy),axis=1 )
...
def div_Hellinger(H1,H2,resolution=1.0):
    '''
    Hellinger.
	Adapted to change the sum into an integral using the trapezoidale rule.
    ...
    return ( 0.5* np.trapz( (np.sqrt(H1) - np.sqrt(H2))**2, dx= resolution, axis=1))
...
def dist_Minkowski(H1,H2,p=2,resolution=1.0):
    ...
    return ( np.trapz((np.abs(H1-H2))**p, dx= resolution, axis=1))**(1.0/p)
...
def dist_SAM(H1, H2, resolution = 1.0):
    ...
    """
	 ...
	 Adapted to transform the sum into an integral using the trapezoidal rule
    ...
    A = np.trapz(H1*H2, dx= resolution, axis=1)/((np.trapz(H1**2, dx= resolution, axis=1)**(1.0/2.0))*(np.trapz(H2**2.0, dx= resolution, axis=1)**(1.0/2.0)))
...
def GlobalSpectralStats(Spectral_Set, Median_Spectrum, Spectral_Ref, Resolution=1.0):
    ...
    Dist2Median = pseudo_div_KL2(Spectral_Set, Median_Spectrum,  Resolution ).sum(axis=1).reshape(count,1)
    Dist2Ref    = pseudo_div_KL2(Spectral_Set, Spectral_Ref,  Resolution ).sum(axis=1).reshape(count,1)

    Dist_Ref2Median = pseudo_div_KL2(Spectral_Ref, Median_Spectrum,  Resolution).sum(axis=1)
...
#    return np.trapz(P*np.sinh(alpha * (np.log(P) - np.log(Q)) ) / np.sinh(alpha), dx=resolution, axis=1)
    return ( np.sum(P*np.sinh(alpha * (np.log(P) - np.log(Q)) ) / np.sinh(alpha),  axis=1) +
             np.sum(Q*np.sinh(alpha * (np.log(Q) - np.log(P)) ) / np.sinh(alpha),  axis=1) )
##============================================================================
...
#    return np.trapz(P*np.sinh(alpha * (np.log(P) - np.log(Q)) ) / np.sinh(alpha), dx=resolution, axis=1)
    return ( np.trapz(P*np.sinh(alpha * (np.log(P) - np.log(Q)) ) / np.sinh(alpha), dx=resolution,  axis=1) +
             np.trapz(Q*np.sinh(alpha * (np.log(Q) - np.log(P)) ) / np.sinh(alpha), dx=resolution, axis=1) )
