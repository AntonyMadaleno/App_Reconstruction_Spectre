...
def save_ENVI_Img( SpecImg, DictData, filename):
    ...
    keyvals = ["lines", "samples", "bands", "data type", "wavelength units", "wavelength"]
    dict_verified = True

    ## Verify that the dictionary as the necessary information
    for key in keyvals:
        if key not in DictData.keys():
            print(f"The Dictionary miss key : {key}")
            dict_verified = False
        
    if (dict_verified == False):
        return
...
def multiSpec2CMF_XYZ(SpecImg, Waves, reflectance = False, ligthning='D65', cmf='data/#_360_830_2deg.npy')  :
    '''
...
#        First_SpecImg_wave = 0 #### To finish for a generic function  !!!!!
    ...
    CMF_X = np.load(cmf.replace('#', 'X'))[First_CIE_wave:Last_CIE_wave] ## X = np.load('data\\x_360_830_2deg.npy')[8,-10] # to reduce to 400:790
    CMF_Y = np.load(cmf.replace('#', 'Y'))[First_CIE_wave:Last_CIE_wave]
    ...
    ## ! ## Potentially np.interp is depreciated and should be replaced by np.interp //ERROR
    ...
    light_Spectrum_interp   = np.interp(Waves[0:Last_SpecImg_wave],Waves_CIE,light_Spectrum)
...
def monoSpec_2_CMF_XYZ(Spectrum, Waves, cmf='data/#_360_830_2deg.npy'):
    '''
...
#        First_SpecImg_wave = 0 #### To finish for a generic function  !!!!!
    ...
    # Load XYZ CMF
    CMF_X = np.load(cmf.replace("#", "X"))
    CMF_Y = np.load(cmf.replace("#", "Y"))
    CMF_Z = np.load(cmf.replace("#", "Z"))
...
def Spec_2_CMF_XYZ(Spec, Waves, reflectance = False, ligthning='D65', cmf='data/#_360_830_2deg.npy') :

    if len(Spec.shape) == 1:
        return monoSpec_2_CMF_XYZ(Spec, Waves, cmf= cmf)
    
    return multiSpec2CMF_XYZ(Spec, Waves, reflectance=reflectance, ligthning=ligthning, cmf= cmf)

##=============================================================================
    
def multiXYZ_2_RGB(ImgXYZ, RGB_space ='sRGB', Gamma=2.2):
    '''
...
def XYZ_2_RGB(XYZ, RGB_space ='sRGB', Gamma=2.2):

    if XYZ.shape[0] == 1:
        return monoXYZ_2_RGB(XYZ, RGB_space = RGB_space, Gamma=Gamma)
    
    return multiXYZ_2_RGB(XYZ, RGB_space = RGB_space, Gamma=Gamma)

##=============================================================================

def multiSpec2CMF_RGB(SpecImg, Waves, reflectance = False, ligthning='D65', RGB_space='sRGB', Gamma=2.2, cmf='data/#_360_830_2deg.npy')  :
    '''
    ...
    '''
    ImgXYZ = multiSpec2CMF_XYZ(SpecImg, Waves, reflectance, ligthning, cmf=cmf)
    ImgRGB = multiXYZ_2_RGB(ImgXYZ, RGB_space, Gamma)
    return ImgRGB

##=============================================================================

def monoSpec2CMF_RGB(Spectrum, Waves, reflectance = False, ligthning='D65', RGB_space='sRGB', Gamma=2.2, cmf='data/#_360_830_2deg.npy')  :
    '''
        create an RGB image using the CIE-RGB values (NR dec2016- Ok)
        require to interpolate the CIE-XYZ 2° and Illuminant data to adapt them to the sensor data
    ...
    '''
    ImgXYZ = monoSpec_2_CMF_XYZ(Spectrum, Waves, cmf=cmf)
    ImgRGB = monoXYZ_2_RGB(ImgXYZ, RGB_space, Gamma)
    return ImgRGB

##=============================================================================

def Spec_2_CMF_RGB(Spec, Waves, reflectance = False, ligthning='D65', RGB_space='sRGB', Gamma=2.2, cmf='data/#_360_830_2deg.npy')  :

    if Spec.shape[0] == 1:
        return monoSpec2CMF_RGB(Spec, Waves, reflectance, ligthning, RGB_space, Gamma, cmf=cmf)
    
    return multiSpec2CMF_RGB(Spec, Waves, reflectance, ligthning, RGB_space, Gamma, cmf=cmf)

###=============================================================================
def multiSpec2RGB_MF(SpecImg, Waves, CMF_Set):
    '''
...
def multiXYZ_2_RGB_Space(ImgXYZ, RGB_space_matrix = None, Gamma=2.2):
    '''
        create an RGB image using the CIE-XYZ values 
        require to select an RGB space (sRGB, Adobe, CIE, ....)
        Based on Bruce Lindbloom equations : http://www.brucelindbloom.com/
...
def XYZ_2_RGB_space(XYZ, RGB_space_matrix, Gamma=2.2):

    if XYZ.shape[0] == 1:
        return monoXYZ_2_RGB_Space(XYZ, RGB_space_matrix = RGB_space_matrix, Gamma=Gamma)
    
    return multiXYZ_2_RGB_Space(XYZ, RGB_space_matrix= RGB_space_matrix, Gamma=Gamma)
